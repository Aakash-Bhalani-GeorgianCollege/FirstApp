...
const SignUpScreen: React.FC<Props> = ({ navigation }) => {
    ...
    const handleSignUp = () => {
        if (!nameIsValid(firstName) || !nameIsValid(lastName)) {
            alert("Please enter a valid name (without invalid characters)");
            return;
        }
        ...
        if (!passwordIsValid(password)) {
            alert("Password must be at least 6 characters and only consist of alphanumeric characters and the following characters: ~!@#$%^&*()_+");
            return;
        }
